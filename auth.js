...
// console.log(process.env.DOORDASH_AUTH_API);

let timeOut = process.env.DOORDASH_AUTH_TOKEN_CRON_TIME;
...
setTimeout(async function () {
  ...
  try {
    let config = {
      method: 'post',
      url: encodeURI(process.env.DOORDASH_AUTH_API),
      headers: {
    ...
    if (response) {
      ...
      let store_auth_token = await deliveryChannelModel.update({ brand_name: 'DoorDash' }, {
